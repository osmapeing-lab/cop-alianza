...
// ═══════════════════════════════════════════════════════════════════════
const ETAPAS_ALIMENTACION = ['Iniciador', 'Levante', 'Engorde', 'Finalización']
...
const PLAN_ALIMENTACION_COMPLEMENTARIO = [
  ...
  { semana: 3,  dia_inicio: 14,  dia_fin: 20,  etapa: 'Preiniciador + leche',     consumo_dia_cerdo_kg: null,  peso_esperado_kg: 5.5 },
  { semana: 4,  dia_inicio: 21,  dia_fin: 27,  etapa: 'Preiniciador + leche',     consumo_dia_cerdo_kg: null,  peso_esperado_kg: 7.0 },
  { semana: 5,  dia_inicio: 28,  dia_fin: 34,  etapa: 'Preiniciador + leche',     consumo_dia_cerdo_kg: null,  peso_esperado_kg: 8.5 },
  { semana: 6,  dia_inicio: 35,  dia_fin: 41,  etapa: 'Preiniciador',             consumo_dia_cerdo_kg: null,  peso_esperado_kg: 10.0 },
  { semana: 7,  dia_inicio: 42,  dia_fin: 48,  etapa: 'Preiniciador',             consumo_dia_cerdo_kg: null,  peso_esperado_kg: 12.5 },
  ...
  <div className="page-dashboard">
    ...
    {(() => {
      ...
      {lotesActivos.map((lote, i) => {
        ...
        if ((lote.peso_inicial_promedio || 0) > 0 && edadManual !== null && edadManual !== undefined) {
        ...
        // Plan completo (Sem 1–26), puntos en el centro de cada semana
        const curva = PLAN_ALIMENTACION
          .filter(s => s.dia_fin >= edadEntrada)
          .map(s => ({ semana: s.semana, dia: Math.round((s.dia_inicio + s.dia_fin) / 2), peso_esperado: s.peso_esperado_kg, fase: s.tipo }))
        ...
        const datos = curva.map(punto => {
          ...
          const tieneP = puntosConEntrada.some(p => Math.abs(p.dia - punto.dia) <= 4)
          return {
            ...
            {(() => {
              ...
              const datosGrafica = curvaRecortada.map(punto => {
                ...
                return {
                  ...
                  tienePesaje: !!puntosPesaje.find(p => Math.abs(p.dia - punto.dia) <= 4),
                  fase: punto.fase
                }
              })
...
{/* ════════════════════════════════════════════════════════════════ */}
          ...
          {pagina === 'pesajes' && (
            <div className="page-pesajes">
              ...
              {/* Báscula oculta — no disponible en esta granja */}
              {false && (
                <div>bascula</div>
              )}
